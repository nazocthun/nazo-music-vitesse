...
export const convertComment = async (res: AxiosResponse<any>): Promise<MusicCommentWrapper> => {
  const commentList = res.data.data.comments
  return {
    comments: commentList.map((item: { user: { userId: any; nickname: any; avatarUrl: any }; commentId: any; content: any; time: any; timeStr: any; likedCount: any; beReplied: { user: { userId: any; nickname: any; avatarUrl: any }; content: any }[] }) => {
      return {
        user: {
          userId: item.user.userId,
          nickname: item.user.nickname,
